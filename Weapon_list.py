...
class Bumer(Weapon):
    ...
    def getDesc(self, damagetaken, user):
        if damagetaken != 0:
            user.weaponeffect.append(self)
            d = str('〽️'+ "|" + getattr(self,str('desc' + str(random.randint(1, 3)))) + "\n♻️Bumerang " + str(((damagetaken*0.6)*10)/10) + ' zarb berib aylanib kelib yana '+ str(((damagetaken*0.3)*10)/10) + ' zarba yetkazdi.\n⚡️' + user.target.name + ' 1 energiya yo`qotayabdi.')
            for a in user.abilities:
                d = a.onhitdesc(a,d,user)
        else:
            d = str('〽️' + "|" + getattr(self,str('desc' + str(random.randint(4, 6)))))
        return d

    desc1 = 'O`yinchi Raqibga aylantirib Bumerang otdi.'
    desc2 = 'O`yinchi Raqibga Bumerang otayabdi.'
    desc3 = 'O`yinchi ko`zlab Raqibga Bumerang otdi.'
    desc4 = 'O`yinchi Raqibga Bumerang otayabdi, lekin tekkiza olmayabdi.'
    desc5 = 'O`yinchi Raqibga Bumerang otayabdi, lekin tekkiza olmayabdi.'
    desc6 = 'O`yinchi Raqibga Bumerang otayabdi, lekin tekkiza olmayabdi.'
...
class Sniper(Weapon):
    ...
    desc5 = ('O`yinchi Raqibiga Snayper Vintovkasils'
             ' bilan otayabdi, lekin tekkiza olmayabdi.')
...
class Astar(Weapon):
    ...
    def get_action(self, p, call):
        ...
        p.turn = call.data
        for c in enemyteam:
            if p.energy + self.bonus + p.accuracy + p.bonusaccuracy >= 20:
                keyboard1.add(types.InlineKeyboardButton(text=c.name, callback_data=str('op' + str(c.chat_id))))
            else:
                keyboard1.add(types.InlineKeyboardButton(text=c.name, callback_data=str('op' + str(c.chat_id))),
                              types.InlineKeyboardButton(text="Energiyani Jamlash", callback_data=str('aim' + str(c.chat_id))))
    ...
    def special(self, user, call):
        if user.aimtarget != call.data[3:]:
            ...
            user.bonusdamage +=2
        if user.energy + self.bonus + user.accuracy + user.bonusaccuracy >= 20:
            bot.send_message(user.chat_id, 'Aniqlik maksimal!')
        user.fight.string.add('✳️' + "|" + user.name + " sehr energiyasini yig'ayabdi.")
        print('scheck')
    ...
    def getDesc(self, damagetaken, user):
        if damagetaken != 0:
            user.weaponeffect.append(self)
            d = str('❎' + "|" + getattr(self,str('desc' + str(random.randint(1, 3)))) + " Yetkazildi " + str(damagetaken) + ' zarb.')
            for a in user.abilities:
                d = a.onhitdesc(a,d,user)
        else:
            d = str('❎' + "|" + getattr(self,str('desc' + str(random.randint(4, 6)))))
        return d


    desc1 = 'O`yinchi ❇️𝔸𝕤𝕥𝕒𝕣𝕥𝕒 ℙ𝕠𝕤𝕒𝕩𝕚🎋 bilan 𝙦𝙪𝙮𝙤𝙨𝙝☀️ energiyasini yig`ib Raqibni kuydirayabdi.'
    desc2 = 'O`yinchi ❇️𝔸𝕤𝕥𝕒𝕣𝕥𝕒 ℙ𝕠𝕤𝕒𝕩𝕚🎋 bilan 𝙤𝙮🌙 energiyasini yig`ib Raqibni harakatsizlantirayabdi.'
    desc3 = 'O`yinchi ❇️𝔸𝕤𝕥𝕒𝕣𝕥𝕒 ℙ𝕠𝕤𝕒𝕩𝕚🎋 bilan 𝙠𝙤𝙞𝙣𝙤𝙩💫 energiyasini yig`ib Raqibni parchalab yuborayabdi.'
    desc4 = 'O`yinchi Raqibiga zarar yetkaza olmayabdi.'
    desc5 = 'O`yinchi Raqibiga zarar yetkaza olmayabdi.'
    desc6 = 'O`yinchi Raqibiga zarar yetkaza olmayabdi.'
...
class Drago(Weapon):
    ...
    def get_action(self, user, call):
        ...
        if user.countercd == 0 and user.energy > 1:
            keyboard1.add(types.InlineKeyboardButton(text="Qizil Xujum", callback_data=str('aim')))
        keyboard1.add(types.InlineKeyboardButton(text='Bekor qilish', callback_data=str('opcancel')))
        bot.send_message(user.chat_id, 'Raqibni tanlang.', reply_markup=keyboard1)

    def special(self, user, call):

        user.fight.string.add("🔥Olovli Drakon🐉 butun maydonni🌋 qizil alanga bilan to`ldirmoqda. Alanga🏜 ichidan tanasi olovda🌫 yonib turgan Qizil Ajdarlar👹 chiqib kelishdi.")
    ...
    desc1 = '🔥Olovli Drakon🐉 Raqibga olov purkayabdi☄️!'
    desc2 = '🔥Olovli Drakon🐉 Raqibga olovli panjalari bilan tashlanayabdi!'
    desc3 = '🔥Olovli Drakon🐉 Raqibga olovli yadrolarni purkayabdi!'
    desc4 = '🔥Olovli Drakon🐉 Raqibga olov purkayabdi☄️, lekin tekkiza olmayabdi.'
    desc5 = '🔥Olovli Drakon🐉 Raqibga olov purkayabdi☄️, lekin tekkiza olmayabdi.'
    desc6 = '🔥Olovli Drakon🐉 Raqibga olov purkayabdi☄️, lekin tekkiza olmayabdi.'
...
class Gauss(Weapon):
    ...
    def getDesc(self, damagetaken, user):
        if damagetaken != 0:
            self.energy_loss = random.randint(2,3)
            user.weaponeffect.append(self)
            d = str('🧪' + "|" + getattr(self,str('desc' + str(random.randint(1, 3)))) + " Yetkazildi " + str(damagetaken) + ' zarar!\n☣️Zaxarlanish darajasi: ' + str(self.energy_loss) + '❕')
            for a in user.abilities:
                d = a.onhitdesc(a,d,user)
        else:
            d = str('🧪' + "|" + getattr(self,str('desc' + str(random.randint(4, 6)))))
    ...
    desc1 = 'O`yinchi Raqibga 🦠Toksik Raffaelloni🍬 beryabdi.'
    desc2 = 'O`yinchi Raqib 🤢Bo`kkanicha Raffaelloni🍬 beryabdi.'
    desc3 = 'O`yinchi Raqibga 🌶Qalampirli Raffaelloni🍬 otayabdi.'
    desc4 = 'O`yinchi Raqibga Toksik Raffaelloni🍬 beryabdi, lekin yedira olmayabdi.'
    desc5 = 'O`yinchi Raqibga Toksik Raffaelloni🍬 beryabdi, lekin yedira olmayabdi.'
    desc6 = 'O`yinchi Raqibga Toksik Raffaelloni🍬 beryabdi, lekin yedira olmayabdi.'
...
class Arxan(Weapon):
    ...
    def getDesc(self, damagetaken, user):
        if damagetaken != 0:
            self.energy_loss = random.randint(1,4)
            user.weaponeffect.append(self)
            d = str('☸️' + "|" + getattr(self,str('desc' + str(random.randint(1, 3)))) + " Yetkazildi " + str(damagetaken) + ' zarar!\n🌩Sexrlanish darajasi: ' + ' ⚡️'*self.energy_loss + '❕')
            for a in user.abilities:
                d = a.onhitdesc(a,d,user)
        else:
            d = str('☸️' + "|" + getattr(self,str('desc' + str(random.randint(4, 6)))))
    ...
    desc1 = 'O`yinchi ✡️𝔸𝕣𝕩𝕒𝕟𝕘𝕖𝕝 ℙ𝕠𝕤𝕒𝕩𝕚🎋 kuchidan foydalanib 🦏𝙺𝚊𝚛𝚔𝚒𝚍𝚘𝚗𝚗𝚒 chaqiryabdi.\n🦏𝙺𝚊𝚛𝚔𝚒𝚍𝚘𝚗 Raqibga qaqshatgich zarba beryabdi.'
    desc2 = 'O`yinchi ✡️𝔸𝕣𝕩𝕒𝕟𝕘𝕖𝕝 ℙ𝕠𝕤𝕒𝕩𝕚🎋 kuchidan foydalanib 🐘𝙵𝚒𝚕𝚗𝚒 chaqiryabdi.\n🐘𝙵𝚒𝚕 Raqibga ayovsiz zarba beryabdi.'
    desc3 = 'O`yinchi ✡️𝔸𝕣𝕩𝕒𝕟𝕘𝕖𝕝 ℙ𝕠𝕤𝕒𝕩𝕚🎋 kuchidan foydalanib yirtqich 🦅𝙱𝚞𝚛𝚐𝚞𝚝𝚗𝚒 chaqiryabdi.\n🦅𝙱𝚞𝚛𝚐𝚞𝚝𝚗𝚒 Raqibga tirnoqlari bilan tashlanayabdi.'
    desc4 = 'O`yinchi Raqibga ✡️Posaxdan zarba bera olmayabdi.'
    desc5 = 'O`yinchi Raqibga qarshi ✡️Posax kuchidan foydalana olmayabdi.'
    desc6 = 'O`yinchi Raqibga qarshi ✡️Posax chaqirig`idan foydalana olmayabdi.'
...
class Dragos(Weapon):
    ...
    def get_action(self, user, call):
        ...
        for c in targets:
            keyboard1.add(types.InlineKeyboardButton(text=c.name, callback_data=str('op' + str(c.chat_id))))
        if user.countercd == 0 and user.energy > 1:
            keyboard1.add(types.InlineKeyboardButton(text="Muzli Xujum", callback_data=str('aim')))
        keyboard1.add(types.InlineKeyboardButton(text='Bekor qilish', callback_data=str('opcancel')))
        bot.send_message(user.chat_id, 'Raqibni tanlang.', reply_markup=keyboard1)

    def special(self, user, call):

        user.fight.string.add("❄️Muzli Drakon🐉 qanotlarini qattiq siltab olamni qaxraton sovuqqa🗻 to'ldirdi. Sovuqdan🌨 kuch olgan Drakonda raqiblar soniga teng miqdorda boshlar🐲 o'sib chiqdi.")
    ...
    def getDesc(self, damagetaken, user):
        if damagetaken != 0:
            ...
            return str('' + "" + getattr(self,str('desc' + str(random.randint(4, 6)))))

    def special_end(self, user):
        if user.turn == 'attack' + str(user.fight.round):
            if user.target.turn == 'reload' + str(user.fight.round) or 'dog_rest' + str(user.fight.round):
                user.target.energy -= 5
                user.fight.string.add('❄️' + user.target.name + ' 5 energiyasi muzlamoqda.')
...
class Tayoqcha(Weapon):
    ...
    def effect(self, user):
        if random.randint(1, 3) <= self.chance:
            if user.target.toughness > 2:
                user.target.toughness -= 1
            if user.target.toughness > 2:
                user.fight.string.add('🔮' + '|' + user.target.name + ' sexrlandi!')
            else:
                user.fight.string.add('🔮' + '|' + user.target.name + ' sexriga boshqa qarshilik qila olmaydi.')
        user.weaponeffect.remove(self)
    ...
    def getDesc(self, damagetaken,user):
        if damagetaken != 0:
            user.weaponeffect.append(self)
            if not self.Melee:
                d = str('🧚‍♂' + "|" + getattr(self,str('desc' + str(random.randint(1, 9)))) + " Yetkazildi " + str(damagetaken) + ' zarba.')
            else:
                d = str('🧚‍♂' + "|" + getattr(self,str('desc' + str(random.randint(1, 9)))) + " Yetkazildi " + str(damagetaken) + ' zarba.')
            for a in user.abilities:
                d = a.onhitdesc(a, d, user)
            ...
            return str('🧚‍♂' + "|" + getattr(self,str('desc' + str(random.randint(10, 12)))))
...
class Posax(Weapon):
    ...
    def hit(self,user):
        ...
        while d != dmax:
            ...
            if x > 10 - user.energy - self.bonus - user.accuracy - user.tempaccuracy + user.target.evasion:
                n += 1
            d += 1
        if n != 0 and random.randint(1,3)< self.chance:
            user.target.firecounter += 1
            user.target.offfire = user.fight.round + 2
            user.Hitability = True
        if n != 0 and random.randint(1,3)< self.chance:
    ...
    def effect(self, user):
        if random.randint(1, 2) <= self.chance:
            if user.target.toughness > 2:
                user.target.toughness -= 1
            if user.target.toughness > 2:
                user.fight.string.add('💀𝐎`𝐥𝐢𝐦 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢 o`rog`i bilan ' + '' + user.target.name + 'ga og`ir kasallik yuqtirmoqda!')
            else:
                user.fight.string.add('💀𝐎`𝐥𝐢𝐦 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢 kasalligi ' + '' + user.target.name + ' tanasini egalladi.')
        user.weaponeffect.remove(self)
    ...
    def getDesc(self, damagetaken,user):
        if damagetaken != 0:
            ...
            if not self.Melee:
                ...
                d = str('🎋' + "|" + getattr(self,str('desc' + str(random.randint(1, 3)))) + " Yetkazildi " + str(damagetaken) + ' zarba.')
            if user.target.firecounter == 1 and user.Hitability:
                d += '\n🏹𝐅𝐚𝐭𝐡 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢 kamoni o`qi alangalanib ' + "|" + user.target.name + 'ni yondirib yubordi!'
            elif user.target.firecounter > 1 and user.Hitability:
                d += '\n🏹𝐅𝐚𝐭𝐡 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢' + "" ' kuchi ortmoqda!'
            for a in user.abilities:
                d = a.onhitdesc(a,d,user)
            if user.target.bleedcounter == 1 and user.Hitability:
                d += '\n🗡𝐔𝐫𝐮𝐬𝐡 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢 qilichi zarbi' + "|" + user.target.name + 'ni qon yo`qottirdi!'
            elif user.target.bleedcounter > 1 and user.Hitability:
                d += '\n🗡𝐔𝐫𝐮𝐬𝐡 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢' + "" ' kuchi ortmoqda!'
            for a in user.abilities:
                d = a.onhitdesc(a, d, user)
            return d
        else:
    ...
    def special_end(self, user):
        if user.turn == 'attack' + str(user.fight.round):
            if user.target.turn == 'reload' + str(user.fight.round) or 'dog_rest' + str(user.fight.round):
                user.target.energy -= 1
                user.fight.string.add('⚖️𝐎𝐜𝐡𝐥𝐢𝐤 𝐜𝐡𝐚𝐯𝐚𝐧𝐝𝐨𝐳𝐢 tarozisi ' + user.target.name + ' kuchini olib qo`ymoqda.')
...
bazuka = Bazuka(6, 2, 6, -1, 0, False, True, True, '☢️Bazooka','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 3, standart=False)
...
nekogun = NekoGun(3, 1, 2, 3, 1, True, False, False, 'NekoGUN💠','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 3, standart=False)
...
thanos = Thanos(1, 1, 2, 2, 0, True, False, False, 'Cheksizlik Qilichi','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 3, standart=False)
...
posax = Posax(2, 1, 3, 2, 0, True, False, False, '☠️Apokalipsis Posaxi⚜️','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 3, standart=False)
posax.desc1 = 'O`yinchi Raqibga qarshi 𝐀𝐩𝐨𝐤𝐚𝐥𝐢𝐩𝐬𝐢𝐬𝐧𝐢 Chavondozlarini chaqirmoqda!'
posax.desc2 = 'O`yinchi 𝐀𝐩𝐨𝐤𝐚𝐥𝐢𝐩𝐬𝐢𝐬 𝐏𝐨𝐬𝐚𝐱𝐢𝐝𝐚𝐧 foydalanib Chazandozlarga Raqibini o`ldirishni buyurdi!'
posax.desc3 = 'O`yinchi 𝐀𝐩𝐨𝐤𝐚𝐥𝐢𝐩𝐬𝐢𝐬 chaqirmoqda. Chavandozlar birin ketin kelib Raqibga xujum qilib boshladi!'
posax.desc4 = 'O`yinchi Raqibga qarshi Apokalipsis Chavondozlarini chaqirmoqchi edi, lekin kuchi yetmadi.'
posax.desc5 = 'O`yinchi Raqibga qarshi Apokalipsis Chavondozlarini chaqirmoqchi edi, lekin javob kelmadi.'
posax.desc6 = 'O`yinchi Raqibga qarshi Apokalipsis Chavondozlarini chaqirmoqchi edi, lekin ular kelmadi.'

knuckles = Knuckles(3, 1, 2, 3, 0, True, False, False, "Kastet",'1-2' + u'\U0001F525' + "|" + '2' + u'\U000026A1', natural=True)
...
club = Club(4, 1, 2, 3, 0, True, False, False, "Excalibur",'1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', standart=False)
...
bumer = Bumer(3, 1, 2, 2, 0, False, False, True, 'Bumerang〽️', '1-3' + u'\U0001F44A' + "|" + '2' + u'\U000026A1', standart=False)
sniper = Sniper(1, 1, 5, -4, 8, False, False, False, 'Snayper Vintovka','8' + u'\U0001F4A5' + "|" + '5' + u'\U000026A1')
astar = Astar(2, 2, 5, 2, 0, False, False, False, '🔯Astarta Posaxi🎋','8' + u'\U0001F4A5' + "|" + '5' + u'\U000026A1', standart=False)
...
drago = Drago(10, 10, 2, 10, 0, False, False, False, 'Olovli Ajdarho Nafasi','1' + '👹' + "|" + '3' + '👹', standart=False, natural=True)
...
knifee = Bleeding(2, 1, 2, 2, 0, True, False, False, 'Lazerli Pushka','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1',6, standart=False)
...
helll = Helling(1, 1, 4, 4, 0, True, False, False, '🔥Jahannam Qilichi🗡','1-2' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 8, standart=False)
helll.desc1 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi!'
helll.desc2 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi!'
helll.desc3 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi!'
helll.desc4 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi, lekin tekkiza olmayabdi.'
helll.desc5 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi, lekin tekkiza olmayabdi.'
helll.desc6 = 'O`yinchi Raqibini 𝕁𝕒𝕙𝕒𝕟𝕟𝕒𝕞 ℚ𝕚𝕝𝕚𝕔𝕙𝕚 bilan urayabdi, lekin tekkiza olmayabdi.'
olovlis = Olovlis(1, 1, 3, 3, 0, False, True, True, '🔸Olovli Kristal🔸','1' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 8, standart=False)
olovlis.desc1 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi!'
olovlis.desc2 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi!'
olovlis.desc3 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi!'
olovlis.desc4 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi, lekin tekkiza sexrlayabdi.'
olovlis.desc5 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi, lekin tekkiza sexrlayabdi.'
olovlis.desc6 = 'O`yinchi Raqibini 🔸Olovli Kristal🔸 bilan sexrlayabdi, lekin tekkiza sexrlayabdi.'
...
tayoqcha = Tayoqcha(4, 1, 2, 2, 0, False, True, True, 'Sehrli Tayoqcha🧚‍♂','1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 8, standart=False)
tayoqcha.desc1 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Expecto Patronum" afsunini ishlatib Raqibni ruxlar olamiga tushirib qo`ydi!'
tayoqcha.desc2 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Accio" afsunini ishlatib Raqibga chaqmoq bilan zarb berdi!'
tayoqcha.desc3 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Crucio" afsunini ishlatib Raqibni azobli og`riqni his qilishga majbur qilyabdi!'
tayoqcha.desc4 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Avada Kedavra" afsunini ishlatib Raqibga o`lim zarbini bermoqda!'
tayoqcha.desc5 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Levicorpus" afsunini ishlatib Raqibni osmonga uchirib yubordi!'
tayoqcha.desc6 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Expelliarmus" afsunini ishlatib Raqibini qurolini tushirib yubordi!'
tayoqcha.desc7 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Reducto" afsunini ishlatib Raqib ustiga atorfdagi jihozlarni tushurib yubordi!'
tayoqcha.desc8 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Imperio" afsunini ishlatib Raqibini o`ziga zarb berishga majbur qildi!'
tayoqcha.desc9 = 'O`yinchi 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan "Riddikulus" afsunini ishlatib Raqibni kulguli holatga keltirib qo`ydi!'
tayoqcha.desc10 = 'O`yinchi Raqibini 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan sexrlayabdi, lekin sexri yetmadi.'
tayoqcha.desc11 = 'O`yinchi Raqibini 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan sexrlayabdi, lekin afsuni kuchsiz chiqdi.'
tayoqcha.desc12 = 'O`yinchi Raqibini 𝗦𝗲𝘅𝗿𝗹𝗶 𝗧𝗮𝘆𝗼𝗾𝗰𝗵𝗮 bilan sexrlayabdi, lekin aniq tekkiza olmayabdi.'
...
magniy = Magniy(10, 30, 2, 10, 0, True, False, False, "O'lim O'rog'i⏳",'1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 4, standart=False, natural=True)
...
magniya = Magniy(3, 2, 2, 3, 0, True, False, False, "☠️O'lim O'rog'i⏳",'1-3' + u'\U0001F525' + "|" + '2' + u'\U000026A1', 4, standart=False, natural=True)
...
iceman = Iceman(1, 1, 0, 10, 0, True, False, True, '🔹Muzli Kristal🔹', '1-4' + u'\U0001F44A' + "|" + '3' +  u'\U000026A1', standart=False)
iceman.desc1 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi.'
iceman.desc2 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi.'
iceman.desc3 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi.'
iceman.desc4 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi, lekin tekkiza olmayabdi.'
iceman.desc5 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi, lekin tekkiza olmayabdi.'
iceman.desc6 = 'O`yinchi Raqibini 🔹Muzli Kristal🔹 bilan sexrlayabdi, lekin tekkiza olmayabdi.'
dragos = Dragos(10, 15, 2, 10, 0, False, False, False, 'Muzli Ajdarho Nafasi', '1-4' + u'\U0001F44A' + "|" + '3' +  u'\U000026A1', standart=False, natural=True)
dragos.desc1 = '❄️Muzli Drakon🐉 Raqibga Sovuq Olov purkayabdi.'
dragos.desc2 = '❄️Muzli Drakon🐉 Raqibga Muz Kristallarini purkayabdi.'
dragos.desc3 = '❄️Muzli Drakon🐉 Raqibga Muzli Panjalari bilan tashlanayabdi.'
dragos.desc4 = '❄️Muzli Drakon🐉 Raqibga sovuq olov purkayabdi, lekin tekkiza olmayabdi.'
dragos.desc5 = '❄️Muzli Drakon🐉 Raqibga sovuq olov purkayabdi, lekin tekkiza olmayabdi.'
dragos.desc6 = '❄️Muzli Drakon🐉 Raqibga sovuq olov purkayabdi, lekin tekkiza olmayabdi.'
...
Shotgunn = Weapon(3, 1, 3, 4, 0, False, True, True, 'ShotGun🧨', '1-3' + u'\U0001F4A5' + "|" + '4' + u'\U000026A1', pellets=True, standart=False)
Shotgunn.desc1 = 'O`yinchi ShotGundan🧨 Raqibni kallasiga otayabdi.'
Shotgunn.desc2 = 'O`yinchi ShotGundan🧨 Raqibni oyog`iga otayabdi.'
Shotgunn.desc3 = 'O`yinchi ShotGundan🧨 Raqibni tanasiga otayabdi.'
Shotgunn.desc4 = 'O`yinchi ShotGundan Raqibiga otayabdi, lekin tekkiza olmayabdi.'
Shotgunn.desc5 = 'O`yinchi ShotGundan Raqibiga otayabdi, lekin tekkiza olmayabdi.'
Shotgunn.desc6 = 'O`yinchi ShotGundan Raqibiga otayabdi, lekin tekkiza olmayabdi.'
Magnumm = Weapon(2, 1, 1, 5, 0, False, False, True, 'Shturm Plyumoti', '2' + u'\U0001F4A5' + "|" + '3' + u'\U000026A1')
Magnumm.desc1 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi.'
Magnumm.desc2 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi.'
Magnumm.desc3 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi.'
Magnumm.desc4 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi, lekin tekkiza olmayabdi.'
Magnumm.desc5 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi, lekin tekkiza olmayabdi.'
Magnumm.desc6 = 'O`yinchi Raqibiga Shturm Plyumotidan otayabdi, lekin tekkiza olmayabdi.'
...
borini = Borini(3, 1, 2, 3, 0, True, True, True, '🐺Bo`ri infeksiyasi🧬', '1-3' + u'\U0001F4A5' + "|" + '2' + u'\U000026A1', 4, standart=False, natural=True)
borini.desc1 = 'O`yinchi bo`riga🐺 aylanib Raqibga tirnoqlari bilan tashlandi.'
borini.desc2 = 'O`yinchi bo`riga🐺 aylanib Raqibni tishlayabdi.'
borini.desc3 = 'O`yinchi bo`riga🐺 aylanib Raqib oyog`ini tishlab oldi.'
borini.desc4 = '🐺Bo`ri O`yinchi Raqibiga tashlanayabdi, lekin tekkiza olmayabdi.'
borini.desc5 = '🐺Bo`ri O`yinchi Raqibiga tashlanayabdi, lekin tekkiza olmayabdi.'
borini.desc6 = '🐺Bo`ri O`yinchi Raqibiga tashlanayabdi, lekin tekkiza olmayabdi.'
gauss = Gauss(3, 2, 3, 2, 0, False, False, False, 'RaffaelGUN', '1-4' + u'\U0001F4A5' + "|" + '2' + u'\U000026A1', standart=False)
arxan = Arxan(4, 2, 3, 2, 0, False, False, False, '❇️Arxangel Posaxi🎋', '1-4' + u'\U0001F4A5' + "|" + '2' + u'\U000026A1', standart=False)
