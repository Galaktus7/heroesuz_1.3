...
class Healt(Item):
    ...
    def use(self, user):
        if user.itemtarget == user:
            user.fight.string.add(u'\U0001F489' + " |" + user.name + ' dori ishlatayabdi.')
        else:
            user.fight.string.add(u'\U0001F489' + " |" + user.name + ' dori berayabdi ' + user.itemtarget.name + 'ga.')
        user.enditems.append(self)
        user.itemlist.remove(self)

    def used(self, user):
        user.itemtarget.hp += 3
        user.Drugged = True
        if user.itemtarget.hp > user.itemtarget.maxhp: user.itemtarget.hp = user.itemtarget.maxhp
        user.fight.string.add(u'\U00002665'*user.itemtarget.hp + u'\U0001F489' + " |" + user.itemtarget.name + ' 3ta jon olayabdi. Qoldi ' + str(user.itemtarget.hp)
                                     + ' jon.')
        user.enditems.remove(self)
        del user.itemtarget
...
raketa = Raketa('Raketa', 'iteme04', standart=False)
firegrenade = Firegrenade('Kokteyl', 'iteme02')
draa = Draa('Drakon nafasi', 'iteme03', standart=False)
...
healt = Healt('Shprits', 'itemt05', standart=False)
...
class ThrowingSpear(Item):
    def useact(self, user):
        ...
        bot.send_message(user.chat_id, 'Nayza uchun nishonni tanlang. Tegish ehtimolligi - ' + str(int(chance)) + '%',reply_markup=keyboard)
    ...
    def use(self, user):
        ...
        if n != 0:
            user.fight.string.add(u'\U0001F4A5' + " |" + user.name + ' kristal nayzani🔹 otayabdi '
                                  + user.itemtarget.name + '. Yetkazildi ' + str(n) + ' zarb!')

        else:
            user.fight.string.add(u'\U0001F4A8' + " |" + user.name + ' kristal nayzani🔹 otayabdi, lekin tekkiza olmayabdi.')
        user.itemlist.remove(self)
        user.throwcd += 3
...
throwspear = ThrowingSpear('Otish', 'itemat9',standart=False)
